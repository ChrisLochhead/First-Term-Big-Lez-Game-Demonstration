...
TextRendering::TextRendering()
{
	...
	FT_Face ft_face;
	if (FT_New_Face(ft_lib, "res/fonts/bigLez.ttf", 0, &ft_face)) cout << "font NOT load" << endl;
	...
	glBindBuffer(GL_ARRAY_BUFFER, CVBO); // 1 квадрат = 6 вершин ( 5 флоат значения на каждую вершину ) 
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * 6 * 5, NULL, GL_DYNAMIC_DRAW); //  GL_DYNAMIC_DRAW - если часто обновляем память
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(GLfloat), 0);
	glEnableVertexAttribArray(1);
...
void TextRendering::draw(string text, glm::vec3 color, float alpha, glm::mat4 matrix)
{
	glUseProgram(textShader->Program);
	glUniformMatrix4fv(glGetUniformLocation(textShader->Program, "PROJECTION_matrix"), 1, GL_FALSE, glm::value_ptr(matrix));
	glUniform3f(glGetUniformLocation(textShader->Program, "text_color"), color.x, color.y, color.z);
	glUniform1f(glGetUniformLocation(textShader->Program, "alpha"), alpha);
